...
export const api = {
  ...
  generateOfficialRun: (caseId) => request(`/admin/cases/${encodeURIComponent(caseId)}/official-run`, { method: "POST", body: "{}" }),
  officialRuns: ({ datasetId = "", search = "" } = {}) => request(`/admin/official-runs?datasetId=${encodeURIComponent(datasetId)}&search=${encodeURIComponent(search)}`),
