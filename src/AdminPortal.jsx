...
function OfficialRuns({ notify }) {
  ...
  const generate = async (item) => { const replace = item.officialRun?.studyStatus === "Official"; const retry = item.officialRun?.studyStatus === "Official failed"; if (replace && !window.confirm("Regenerate this Official Run? The existing answer will be archived and its assessments will remain separate.")) return; setBusyId(item.id); try { if (retry) await api.retryRun(item.officialRun.id); else await api.generateOfficialRun(item.id); notify(retry ? "Official Run retry started." : replace ? "Replacement Official Run started; the prior answer will be archived only after completion." : "Official Run generation started."); refresh(); } catch (error) { notify(error.message); } finally { setBusyId(""); } };
