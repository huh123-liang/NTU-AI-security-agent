...
function CasesPage({ datasetId, navigate }) {
  ...
  return <div className="content-page"><PageHeader eyebrow="CASE LIBRARY" title="Validated clinical cases" copy="Choose an Admin-approved patient record. Single visits, short histories, standard longitudinal cases and undated snapshots retain source lineage." actions={<select className="dataset-select" value={selectedId} onChange={(event) => setSelectedId(event.target.value)}>{datasets.map((dataset) => <option key={dataset.id} value={dataset.id}>{dataset.name}</option>)}</select>} />
...
function Workspace({ caseId, initialRunId, notify, scoringSuspended }) {
  ...
  return <div className="workspace-shell"><div className="workflow-bar"><span className="complete"><b>1</b><i>Case context<small>{titleCase(taskLabel)} evidence policy</small></i></span><span className={runComplete ? "complete" : "active"}><b>2</b><i>Official AI response<small>Pre-generated, blinded and fixed</small></i></span><span className={runComplete ? "active" : ""}><b>3</b><i>Evaluation<small>Score and explain</small></i></span><span className={assessment?.status === "Submitted" ? "complete" : ""}><b>4</b><i>Submit<small>Editable until final lock</small></i></span></div>
    <div className="workspace-grid"><section className="workspace-column patient-column"><div className="column-heading"><span>{realWorld ? "DE-IDENTIFIED REAL-WORLD PATIENT DATA" : "SIMULATED PATIENT DATA"}</span><button className="text-button" onClick={() => setRawEvidenceTarget({ visitNumber: visits.at(-1)?.visit_number || 1, jsonPath: `$.visits[${Math.max(0, visits.length - 1)}]` })}><Eye size={14} />Clinical record</button></div><PatientContext patient={caseData} visits={visits} selectedEvidence={selectedEvidence} /></section>
      <section className="workspace-column response-column"><div className="column-heading"><span>OFFICIAL AI RESPONSE · {titleCase(taskLabel)}</span><div className="run-controls"><StatusBadge>{selectedRun?.modelVersion || "Blinded model"}</StatusBadge><StatusBadge tone={selectedRun?.studyStatus === "Official" ? "success" : "warning"}>{selectedRun?.studyStatus || "Awaiting Admin"}</StatusBadge></div></div>{error && <div className="inline-alert"><Warning size={16} />{error}</div>}{selectedRun ? <ResponsePanel run={selectedRun} selectedEvidence={selectedEvidence} onSelectEvidence={setSelectedEvidence} onOpenRaw={(evidence) => setRawEvidenceTarget(evidence)} /> : <EmptyState icon={Robot} title="Official response pending" copy="An administrator must pre-generate a fixed blinded response before independent doctor scoring begins." />}</section>
