...
import {
  ArrowRight, ArrowsClockwise, Brain, ChartLineUp, Check, ClipboardText, CloudArrowUp, Database,
...
function PatientContext({ patient, visits }) {
  ...
  return <div className="patient-scroll"><article className="patient-summary"><div className="patient-identity"><span className="patient-avatar large">{patient.patientId.slice(-2)}</span><div><h2>{patient.patientId}</h2><p>{patient.age || "—"}-year-old · {titleCase(patient.sex)} · {titleCase(patient.ethnicity)}</p><StatusBadge>Synthetic</StatusBadge></div></div><dl><div><dt>Dataset</dt><dd>{patient.datasetName}</dd></div><div><dt>Reference visit</dt><dd>{shortDate(latest.date)}</dd></div></dl><div className="diagnosis-block"><b>Conditions</b><ul>{patient.conditions.map((condition) => <li key={condition.key || condition.display}>{condition.display || titleCase(condition.key)}</li>)}</ul></div></article>
...
function ReviewCockpit({ draft, setDraft, assessment, saving, onSave }) {
  ...
  return <div className="review-scroll"><div className="overall-score"><div className="score-ring"><ResponsiveContainer width="100%" height="100%"><RadialBarChart innerRadius="76%" outerRadius="100%" data={[{ value: overall * 20, fill: "#c8102e" }]} startAngle={90} endAngle={-270}><RadialBar dataKey="value" cornerRadius={3} background={{ fill: "#e9eef3" }} /></RadialBarChart></ResponsiveContainer><strong>{overall ? overall.toFixed(1) : "—"}<small>/5</small></strong></div><div><span>OVERALL ASSESSMENT</span><h2>{overall >= 4 ? "Good" : overall >= 3 ? "Moderate" : overall ? "Needs attention" : "Not scored"}</h2><p>{scores.length} of {CRITERIA.length} dimensions completed.</p></div>{locked && <StatusBadge tone="danger">Locked</StatusBadge>}</div>
    <div className="rubric-intro"><b>Rubric</b><span>1 = Poor · 5 = Excellent</span></div><div className="criteria-list">{CRITERIA.map((criterion, index) => { const value = draft.criteria[criterion.key]; return <article className="criterion-card" key={criterion.key}><div className="criterion-title"><span>{index + 1}</span><div><b>{criterion.label}</b><small>{criterion.description}</small></div><InfoTip>Feedback is optional and visible only to Admin.</InfoTip></div><div className="score-buttons" role="radiogroup" aria-label={criterion.label}>{[1, 2, 3, 4, 5].map((score) => <button disabled={locked} key={score} className={value.score === score ? "selected" : ""} onClick={() => updateCriterion(criterion.key, { score })}>{score}</button>)}</div><div className="criterion-feedback"><div className="feedback-tags">{criterion.tags.map((tag) => <button disabled={locked} key={tag} className={value.tags.includes(tag) ? "selected" : ""} onClick={() => updateCriterion(criterion.key, { tags: value.tags.includes(tag) ? value.tags.filter((item) => item !== tag) : [...value.tags, tag] })}>{value.tags.includes(tag) && <Check size={11} />}{tag}</button>)}{value.customTags.map((tag) => <button disabled={locked} className="selected custom" key={tag} onClick={() => updateCriterion(criterion.key, { customTags: value.customTags.filter((item) => item !== tag) })}>{tag}<X size={11} /></button>)}</div><div className="custom-tag-row"><input disabled={locked} value={customInputs[criterion.key] || ""} onChange={(event) => setCustomInputs({ ...customInputs, [criterion.key]: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter" && event.currentTarget.value.trim()) { event.preventDefault(); const tag = event.currentTarget.value.trim(); updateCriterion(criterion.key, { customTags: [...new Set([...value.customTags, tag])] }); setCustomInputs({ ...customInputs, [criterion.key]: "" }); } }} placeholder="+ Add custom feedback tag" /></div><textarea disabled={locked} value={value.feedback} onChange={(event) => updateCriterion(criterion.key, { feedback: event.target.value })} placeholder={`Optional feedback on ${criterion.label.toLowerCase()}…`} /></div></article>; })}</div>
