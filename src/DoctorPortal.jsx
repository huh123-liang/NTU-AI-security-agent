...
function DatasetsPage({ navigate, notify }) {
  ...
  const refresh = () => Promise.all([api.datasets(), api.ingestionJobs()]).then(([datasets, ingestion]) => { setItems(datasets.items); setJobs(ingestion.items); }).finally(() => setLoading(false));
  useEffect(() => { refresh(); }, []);
  useEffect(() => {
  ...
  return <div className="content-page"><PageHeader eyebrow="DATA GOVERNANCE" title="Datasets" copy="Your uploads remain private to you and the administrator. Approved system cohorts are available to every doctor." actions={<button className="primary-button" onClick={() => setUploadOpen(true)}><UploadSimple size={17} />Upload dataset</button>} />
    {loading ? <LoadingState label="Loading datasets…" /> : <><section className="surface ingestion-task-panel"><div className="section-heading"><div><span className="eyebrow">BACKGROUND PREPROCESSING</span><h2>Upload and approval status</h2><p>Large hospital CSV bundles continue locally after this page is closed.</p></div></div>{jobs.length ? <div className="ingestion-task-list">{jobs.map((job) => <button key={job.id} onClick={async () => setSelectedJob((await api.ingestionJob(job.id)).job)}><span className="ingestion-stage-icon"><ArrowsClockwise className={["Inspecting", "Processing"].includes(job.status) ? "spin" : ""} size={18} /></span><span><b>{job.name}</b><small>{job.sourceFilename} · {job.stage.replaceAll("_", " ")}</small></span><div className="ingestion-progress"><i style={{ width: `${job.progress}%` }} /><small>{job.progress}%</small></div><StatusBadge tone={job.status === "Failed" ? "danger" : job.status === "Approved" ? "success" : "warning"}>{job.status}</StatusBadge></button>)}</div> : <p className="muted">No preprocessing jobs have been submitted.</p>}</section><section className="surface data-table"><div className="table-head dataset-row"><span>Dataset</span><span>Owner / access</span><span>Quality</span><span>Status</span><span /></div>{items.map((dataset) => <div className="table-row dataset-row" key={dataset.id}><div className="cell-title"><Database size={20} /><span><b>{dataset.name}</b><small>{dataset.sourceFilename}</small></span></div><div><b>{dataset.ownerName}</b><small>{dataset.visibility === "shared" ? "Approved shared cohort" : "Private to uploader + Admin"}</small></div><div><b>{dataset.validCount} valid</b><small>{dataset.quarantinedCount} quarantined · {dataset.declaredCount} declared</small></div><div><StatusBadge>{dataset.status}</StatusBadge></div><div className="row-actions"><button className="text-button" onClick={() => inspect(dataset.id)}><Eye size={15} />Quality</button><button className="secondary-button compact" disabled={!dataset.validCount || dataset.status !== "Approved"} onClick={() => navigate("cases", dataset.id)}>Open<ArrowRight size={14} /></button></div></div>)}</section></>}
    {uploadOpen && <UploadDatasetModal onClose={() => setUploadOpen(false)} onDone={(result) => { setUploadOpen(false); refresh(); notify(result.job ? "Upload completed. Local schema discovery is running; Admin approval will be required." : `Dataset imported: ${result.valid} valid, ${result.quarantined} quarantined`); }} />}
    {selected && <DatasetQualityModal data={selected} onClose={() => setSelected(null)} />}
    {selectedJob && <IngestionStatusModal job={selectedJob} onClose={() => setSelectedJob(null)} />}
  </div>;
}
...
function UploadDatasetModal({ onClose, onDone }) {
  ...
  const submit = async () => {
    ...
    try {
      if (file.name.toLowerCase().endsWith(".zip")) {
        ...
        const result = await api.uploadDataset({ fileName: file.name, name: name || file.name.replace(/\.[^.]+$/, ""), description, contentBase64: await fileToBase64(file) });
        onDone(result);
      }
    }
  ...
  return <Modal title="Upload a clinical dataset" copy="Hospital ZIP bundles may contain multiple CSV or CSV.GZ tables. Standard JSON and CSV remain supported for smaller prepared cohorts." onClose={onClose} wide><div className="upload-zone"><FileArrowUp size={38} /><h3>{file ? file.name : "Choose a dataset file"}</h3><p>{file ? `${(file.size / 1024 / 1024).toFixed(2)} MB · ${file.name.toLowerCase().endsWith(".zip") ? "resumable background preprocessing" : "standard local validation"}` : "The immutable source is retained locally and fingerprinted with SHA-256."}</p>{progress && <div className="upload-progress"><i style={{ width: `${progress.percent}%` }} /><span>{progress.stage === "uploading" ? `Uploading ${progress.percent}%` : "Upload complete · discovering tables"}</span></div>}<label className="secondary-button file-picker">Select ZIP, JSON or CSV<input type="file" accept=".zip,.json,.csv" onChange={(event) => { const next = event.target.files?.[0]; setFile(next); setProgress(null); if (next && !name) setName(next.name.replace(/\.[^.]+$/, "")); }} /></label></div><div className="form-grid"><label>Dataset name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Longitudinal chronic-care cohort" /></label><label>Description<textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Purpose, source and limitations…" /></label></div><div className="import-policy"><ShieldCheck size={18} /><span><b>Private and local by default</b><small>Raw hospital data is processed locally, never sent to DeepSeek, and remains unavailable until Admin approval.</small></span></div>{error && <div className="form-error"><Warning size={17} />{error}</div>}<div className="modal-actions"><button className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={!file || busy} onClick={submit}>{busy ? <><ArrowsClockwise className="spin" size={16} />{progress?.stage === "uploading" ? "Uploading…" : "Starting discovery…"}</> : <><UploadSimple size={16} />Start local preprocessing</>}</button></div></Modal>;
}
...
function CasesPage({ datasetId, navigate }) {
  ...
  return <div className="content-page"><PageHeader eyebrow="CASE LIBRARY" title="Validated longitudinal cases" copy="Choose a patient record. Every visible case passed structural validation and retains its original evidence." actions={<select className="dataset-select" value={selectedId} onChange={(event) => setSelectedId(event.target.value)}>{datasets.map((dataset) => <option key={dataset.id} value={dataset.id}>{dataset.name}</option>)}</select>} />
    <div className="list-toolbar"><label><MagnifyingGlass size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search patient ID or condition" /></label><span>{total} eligible cases</span></div>{loading ? <LoadingState label="Loading validated cases…" /> : <section className="surface data-table"><div className="table-head case-row"><span>Patient</span><span>Longitudinal context</span><span>Demographics</span><span>Evidence</span><span /></div>{cases.map((item) => <button className="table-row case-row clickable" key={item.id} onClick={() => navigate("workspace", item.id)}><div className="cell-title"><span className="patient-avatar">{item.patientId.slice(-2)}</span><span><b>{item.patientId}</b><small>{item.recordType === "deidentified_real_world" ? "De-identified real-world record" : "Synthetic patient"}</small></span></div><div><b>{item.condition}</b><small>{item.visits} completed visits</small></div><div><b>{item.ageTopCoded ? "Age 91+" : `${item.age || "—"} years`} · {titleCase(item.sex)}</b><small>{titleCase(item.ethnicity)}</small></div><div><StatusBadge>Validated</StatusBadge><small>SHA {item.sourceSha256.slice(0, 8)}…</small></div><ArrowRight size={16} /></button>)}</section>}
  </div>;
}
...
function PatientContext({ patient, visits, selectedEvidence }) {
  ...
  return <div className="patient-scroll"><article className="patient-summary"><header className="patient-summary-head"><div className="patient-identity"><span className="patient-avatar large">{patient.patientId.slice(-2)}</span><div><span className="summary-kicker">PATIENT RECORD</span><h2>{patient.patientId}</h2><p>{patient.ageTopCoded ? "Age 91+" : `${patient.age || "—"}-year-old`} · {titleCase(patient.sex)} · {titleCase(patient.ethnicity)}</p></div></div><StatusBadge tone={realWorld ? "success" : undefined}>{realWorld ? "De-identified" : "Synthetic"}</StatusBadge></header><dl className="patient-meta-strip"><div><dt>Dataset</dt><dd title={patient.datasetName}>{patient.datasetName}</dd></div><div><dt>Reference visit</dt><dd>{shortDate(latest.date)}</dd></div><div><dt>History</dt><dd>{visits.length} longitudinal visits</dd></div></dl><div className="diagnosis-block"><div className="diagnosis-heading"><b>Active conditions</b><span>{patient.conditions.length}</span></div><ul>{patient.conditions.map((condition) => <li key={condition.key || condition.display}>{condition.display || titleCase(condition.key)}</li>)}</ul></div></article>
    ...
    <article className="trend-card"><div className="subsection-title"><b>Longitudinal trends</b><span>{availableMetrics.length} available source metric{availableMetrics.length === 1 ? "" : "s"}</span></div>{availableMetrics.length ? <div className="small-multiple-grid">{availableMetrics.map((metric) => <TrendMini key={metric.key} metric={metric} visits={visits} selectedEvidence={selectedEvidence} />)}</div> : <p>No longitudinal measurement series is available in this source record.</p>}<div className="trend-legend"><small>Only source-backed measurements are plotted. Missing laboratory series are not reconstructed.</small></div></article>
