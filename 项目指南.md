...
> 用途：供下一次 Codex 任务或新开发者快速恢复项目状态。更新时间：2026-08-18。工作目录：`C:\Users\Lenovo\Desktop\NTU\NTU AI security agents\MVP2`。
...
- 当前 Prompt 版本：`longitudinal-visit10-plan-v1`。
...
- `agent_runs`：Provider、模型、Prompt、输入快照、自由文本输出、Usage、错误。
...
Workspace 保留参考设计的深色全高左侧栏和三列 Safety Review Cockpit：

1. 左列：患者摘要、10 次纵向 Timeline、趋势图和 Raw Evidence 入口；
2. 中列：模型运行选择、新运行生成、AI 第 10 次计划、模型/Prompt 版本；
...
- 自动测试 `npm test`：7/7 通过，覆盖密码哈希、369/131 数据质量、三 Doctor 汇总、零权重语义、锁定和 Sites 打包。
- `npm run build` 成功。

## 9. 启动、停止与配置

Windows 用户：双击 `Start-Platform.cmd` 或 `一键启动-AI医疗评估平台.cmd`。桌面快捷方式名为 `AI医疗评估平台-MVP2`。启动器会先识别已运行实例，仅在源代码更新后自动 Build，以毫秒级探测选择 4190–4199 的可用端口，在后台启动 SQLite/API 服务并打开浏览器。启动诊断日志保存在 `.runtime/platform.log` 和 `.runtime/platform-error.log`。双击 `Stop-Platform.cmd` 停止由启动器创建的后台进程。
...
DeepSeek 配置写入不进 Git 的 `.env.local`：
